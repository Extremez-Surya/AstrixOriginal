...
module.exports = {
  name: "emergencypanel",
  category: "Moderation",
  description: "Open the server emergency override control panel.",
  ...
  async execute(client, interaction) {
    ...
    const buildPanel = (statusMessage = "Awaiting command deployment. Highly restricted control area.") => {
      ...
      const row1 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("ep_lock_all").setLabel("Lock All").setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId("ep_unlock_all").setLabel("Unlock All").setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId("ep_hide_all").setLabel("Hide All").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId("ep_unhide_all").setLabel("Unhide All").setStyle(ButtonStyle.Primary)
      );

      const row2 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("ep_slow_all_10").setLabel("Slowmode All 10s").setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId("ep_purge_current_10").setLabel("Purge 10 (Here)").setStyle(ButtonStyle.Danger)
      );
    ...
    collector.on("collect", async (i) => {
      if (!i.member.permissions.has("Administrator")) {
        return i.reply({
          content: "You do not have Administrator permissions to deploy emergency overrides.",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        switch (i.customId) {
          ...
          case "ep_hide_all":
            for (const [id, ch] of channels) {
              await ch.permissionOverwrites.edit(everyoneRole, { ViewChannel: false }).catch(() => {});
            }
            status = "Hided all text channels server-wide.";
            break;
          case "ep_unhide_all":
            for (const [id, ch] of channels) {
              await ch.permissionOverwrites.edit(everyoneRole, { ViewChannel: null }).catch(() => {});
            }
    ...
    collector.on("end", async () => {
      try {
        ...
        const disabledPanel = new ContainerBuilder()
          .addTextDisplayComponents(new TextDisplayBuilder().setContent(content));

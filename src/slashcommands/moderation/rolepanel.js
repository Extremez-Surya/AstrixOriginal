...
module.exports = {
  name: "rolepanel",
  category: "Moderation",
  description: "Open the role management panel for a role.",
  ...
  async execute(client, interaction) {
    ...
    if (role.position >= guild.members.me.roles.highest.position) {
      return interaction.editReply("I cannot manage this role because it is higher than or equal to my highest role.").catch(() => null);
    }

    const commandStartTime = Date.now();
    let showRoleInfo = false;

    const buildPanel = (statusMessage = null, actionStartTime = commandStartTime, disabled = false) => {
      ...
      content.push(
        "",
        statusMessage ? `**Status:** ${statusMessage}` : "Choose an action below.",
        "",
        `-# Executed in ${execTime}s`
      );

      const row1 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("rp_add_user").setLabel("Add to User").setStyle(ButtonStyle.Success).setDisabled(disabled),
        new ButtonBuilder().setCustomId("rp_remove_user").setLabel("Remove from User").setStyle(ButtonStyle.Danger).setDisabled(disabled),
        new ButtonBuilder().setCustomId("rp_role_info").setLabel(showRoleInfo ? "Hide Role Info" : "Role Info").setStyle(ButtonStyle.Secondary).setDisabled(disabled)
      );

      const row2 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("rp_add_humans").setLabel("Add to Humans").setStyle(ButtonStyle.Primary).setDisabled(disabled),
        new ButtonBuilder().setCustomId("rp_add_bots").setLabel("Add to Bots").setStyle(ButtonStyle.Primary).setDisabled(disabled),
        new ButtonBuilder().setCustomId("rp_remove_all").setLabel("Remove from All").setStyle(ButtonStyle.Danger).setDisabled(disabled)
      );
    ...
    collector.on("collect", async (i) => {
      ...
      try {
        if (action === "rp_add_user" || action === "rp_remove_user") {
          ...
          const rawInput = submitted.fields.getTextInputValue("user_input").trim();
          const cleanId = rawInput.replace(/[<@!>]/g, "");

          const targetMember = await guild.members.fetch(cleanId).catch(() => guild.members.cache.get(cleanId));
          ...
          if (!targetMember) {
            ...
            if (isAdd) {
              if (targetMember.roles.cache.has(role.id)) {
                ...
                await targetMember.roles.add(role, `Role Panel: Added by ${i.user.tag}`);
              ...
              if (!targetMember.roles.cache.has(role.id)) {
                ...
                await targetMember.roles.remove(role, `Role Panel: Removed by ${i.user.tag}`);
          ...
          status = showRoleInfo ? "Displayed role details." : "Hidden role details.";
        } else {
          await i.deferUpdate().catch(() => null);
          ...
          if (action === "rp_add_humans") {
            const targetMembers = guildMembers.filter((m) => !m.user.bot && !m.roles.cache.has(role.id));
            let count = 0;
            for (const [_, m] of targetMembers) {
              try {
                if (role.position < guild.members.me.roles.highest.position) {
                  await m.roles.add(role, `Role Panel: Bulk add humans by ${i.user.tag}`);
                  count++;
                }
              } catch (err) {}
            }
            ...
            for (const [_, m] of targetMembers) {
              try {
                if (role.position < guild.members.me.roles.highest.position) {
                  await m.roles.add(role, `Role Panel: Bulk add bots by ${i.user.tag}`);
                  count++;
                }
              } catch (err) {}
            }
            ...
            const targetMembers = guildMembers.filter((m) => m.roles.cache.has(role.id));
            let count = 0;
            for (const [_, m] of targetMembers) {
              try {
                if (role.position < guild.members.me.roles.highest.position) {
                  await m.roles.remove(role, `Role Panel: Bulk remove from all by ${i.user.tag}`);
                  count++;
                }
              } catch (err) {}
            }
    ...
    collector.on("end", async () => {
      try {
        const expiredContainer = buildPanel("This panel has expired and is now inactive.", commandStartTime, true);
        await interaction.editReply({

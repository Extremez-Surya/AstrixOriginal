...
module.exports = {
  ...
  description: "View the developer's credentials and server details.",
  ...
  async execute(client, interaction) {
    ...
    const generateDevCard = async (dev) => {
      ...
      try {
        const bgImage = await loadImage(path.join(__dirname, "../../assets/ping_bg.png"));
        ctx.drawImage(bgImage, 0, 0, 900, 350);
      } catch (e) {
        try {
          const bgImage = await loadImage(path.join(__dirname, "../../assets/astrix.png"));
          ctx.drawImage(bgImage, 0, 0, 900, 350);
        } catch (err) {
      ...
      const len = 15;
      // Top Left
      ctx.beginPath(); ctx.moveTo(cardX + 10, cardY + 10 + len); ctx.lineTo(cardX + 10, cardY + 10); ctx.lineTo(cardX + 10 + len, cardY + 10); ctx.stroke();
      // Top Right
      ctx.beginPath(); ctx.moveTo(cardX + cardW - 10 - len, cardY + 10); ctx.lineTo(cardX + cardW - 10, cardY + 10); ctx.lineTo(cardX + cardW - 10, cardY + 10 + len); ctx.stroke();
      // Bottom Left
      ctx.beginPath(); ctx.moveTo(cardX + 10, cardY + cardH - 10 - len); ctx.lineTo(cardX + 10, cardY + cardH - 10); ctx.lineTo(cardX + 10 + len, cardY + cardH - 10); ctx.stroke();
      // Bottom Right
      ctx.beginPath(); ctx.moveTo(cardX + cardW - 10 - len, cardY + cardH - 10); ctx.lineTo(cardX + cardW - 10, cardY + cardH - 10); ctx.lineTo(cardX + cardW - 10, cardY + cardH - 10 - len); ctx.stroke();
      ...
      let avatarImage;
      try {
        avatarImage = await loadImage(devUser.displayAvatarURL({ extension: "png", size: 256 }));
      } catch (e) {
        avatarImage = null;
      }
      ...
      ctx.fillText("SYS.LOC: //ROOT/DEVS/VINAY", cardX + 20, cardY + cardH - 22);
      ctx.fillText("SECURE PROTOCOL ACT-980", cardX + cardW - 160, cardY + cardH - 22);
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "prev" && currentPage > 0) {
        currentPage--;
      } else if (i.customId === "next" && currentPage < fetchedTeam.length - 1) {
        currentPage++;
      }

      const { container: newContainer, bannerAttachment: newAttachment } = await buildDevPage(currentPage);
    ...
    collector.on("end", async () => {
      try {
        const { container: finalContainer, bannerAttachment: finalAttachment } = await buildDevPage(currentPage, true);
        await interaction.editReply({

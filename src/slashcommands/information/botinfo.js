...
module.exports = {
  ...
  description: "View comprehensive information and statistics about Astrix.",
    ...
    const buildDashboardPage = (pageIndex, disableAll = false) => {
      ...
      if (pageIndex === 0) {
        ...
        const teamLines = fetchedTeam.map(
          (member) =>
            `> 👑 **${member.role}** ─ ${member.username} (\`${member.id}\`)`,
        );
        content = [
          `### 👥 Astrix Developer Team`,
      ...
      const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `-# *⚡ Live Ping: \`${client.ws.ping}ms\` • Powered by ASTRIXCODE™ • © 2026*`,
          ),
        )

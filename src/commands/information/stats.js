  ...
  async execute(client, message, args) {
    ...
    for (const memberInfo of teamList) {
      try {
        ...
        console.error(
          `Failed to fetch team member with ID ${memberInfo.id}:`,
          e,
        );
      }
    }
    ...
    const buildStatsPage = (activeTab, disableButtons = false) => {
      ...
      if (activeTab === "overview") {
        ...
        fetchedTeam.forEach((member) => {
          ...
          lines.push(
            `   ${gray}L  ${white}${roleStr}${cyan}:${reset}  ${green}${member.username}${reset}`,
          );
        });
        lines.push("```");
    ...
    collector.on("collect", async (i) => {
      ...
      else if (i.customId === "stats_team" || i.customId === "stats_team_back")
        currentTab = "team";

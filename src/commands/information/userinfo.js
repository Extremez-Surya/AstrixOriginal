...
module.exports = {
  ...
  desc: "View detailed information and assets for a server member.",
  ...
  async execute(client, message, args) {
    ...
    const buildUserInfoPage = async (pageIndex, disableComponents = false) => {
      ...
      if (pageIndex === 0) {
        ...
        if (!member) {
          ...
          if (flags.has("ManageMessages"))
            keyPermissions.push("Manage Messages");
          if (flags.has("MentionEveryone"))
        ...
        if (presence?.clientStatus) {
          const clients = Object.keys(presence.clientStatus).map(
            (c) =>
              `${c.charAt(0).toUpperCase() + c.slice(1)} (${presence.clientStatus[c]})`,
          );
    ...
    const replyMsg = await message.reply(replyOptions);
    ...
    collector.on("collect", async (i) => {
      ...
      await i.editReply(updateOptions);
    });

...
module.exports = {
  alias: ["massban", "mban"],
  category: "Moderation",
  desc: "Ban multiple members or user IDs in a single command.",
  ...
  async execute(client, message, args) {
    if (!args[0]) {
      return message.reply("Usage: `.massban <@user1|id1> <@user2|id2> ... [reason]`");
    }

    const userIds = [];
    let reason = "Massban executed by administrator";
    ...
    if (userIds.length > 20) {
      return message.reply("❌ For safety, massban is limited to a maximum of 20 users per execution.");
    }
    ...
    for (const id of userIds) {
      ...
      try {
        await message.guild.bans.create(id, { reason: `[Massban] ${reason} (by ${message.author.tag})` });
        success.push(`<@${id}>`);
        moderationManager.addCase(message.guild.id, {
          action: "BAN",
          targetId: id,
          targetTag: targetMember?.user?.username || id,
    ...
    const container = new ContainerBuilder()
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `### 🔨 **Mass Ban Execution Report**\n` +
          `-# *Processed ${userIds.length} targets*\n\n` +
          (success.length > 0 ? `**Successfully Banned (${success.length}):**\n${success.join(", ")}\n\n` : "") +

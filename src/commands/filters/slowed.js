...
module.exports = {
  alias: ["slowed", "slowedreverb"],
  category: "Filters",
  desc: "Toggle Slowed + Reverb audio filter.",
  ...
  async execute(client, message) {
    ...
    const active = currentFilter === "Slowed + Reverb";

    if (active) {
      await DSP_FILTERS.clear.apply(player);
    } else {
      await DSP_FILTERS.slowed.apply(player);
    }

    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `🌌 **Slowed + Reverb Filter ${!active ? "Enabled 🪐" : "Disabled 🔇"}**`
      )
    );

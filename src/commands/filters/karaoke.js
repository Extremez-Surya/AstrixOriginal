...
module.exports = {
  alias: ["karaoke", "vocalfilter"],
  category: "Filters",
  desc: "Toggle Karaoke vocal reduction filter.",
  ...
  async execute(client, message) {
    ...
    const active = currentFilter === "Karaoke";

    if (active) {
      await DSP_FILTERS.clear.apply(player);
    } else {
      await DSP_FILTERS.karaoke.apply(player);
    }

    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `🎤 **Karaoke Vocal Filter ${!active ? "Enabled 🎶" : "Disabled 🔇"}**`
      )
    );

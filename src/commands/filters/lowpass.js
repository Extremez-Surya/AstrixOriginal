...
module.exports = {
  alias: ["lowpass", "softmuffled"],
  category: "Filters",
  desc: "Toggle Lowpass audio filter.",
  ...
  async execute(client, message) {
    ...
    const active = currentFilter === "Lowpass";

    if (active) {
      await DSP_FILTERS.clear.apply(player);
    } else {
      await DSP_FILTERS.lowpass.apply(player);
    }

    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `🔈 **Lowpass Muffled Filter ${!active ? "Enabled 🔉" : "Disabled 🔇"}**`
      )
    );

...
module.exports = {
  alias: ["rotation", "panrotate"],
  category: "Filters",
  desc: "Toggle 360° stereo sound Rotation filter.",
  ...
  async execute(client, message) {
    ...
    const active = currentFilter === "Rotation 360°";

    if (active) {
      await DSP_FILTERS.clear.apply(player);
    } else {
      await DSP_FILTERS.rotation.apply(player);
    }

    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `🔄 **360° Audio Rotation ${!active ? "Enabled 🌀" : "Disabled 🔇"}**`
      )
    );

...
module.exports = {
  alias: ["vaporwave", "vw"],
  category: "Filters",
  desc: "Toggle Vaporwave aesthetic pitch and tremolo filter.",
  ...
  async execute(client, message) {
    ...
    const active = currentFilter === "Vaporwave";

    if (active) {
      await DSP_FILTERS.clear.apply(player);
    } else {
      await DSP_FILTERS.vaporwave.apply(player);
    }

    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `🌆 **Vaporwave Filter ${!active ? "Enabled 🌴" : "Disabled 🔇"}**`
      )
    );

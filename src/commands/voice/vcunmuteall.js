...
module.exports = {
  alias: ["vcunmuteall", "vunmuteall", "voiceunmuteall"],
  category: "Voice",
  desc: "Server unmute all human members connected to your voice channel.",
  ...
  async execute(client, message, args) {
    ...
    const targets = Array.from(channel.members.values()).filter(
      (m) => !m.user.bot && m.voice.serverMute
    );

    if (targets.length === 0) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`ℹ️ No server-muted human members found in **${channel.name}**.`)
      );
      return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 }).catch(() => null);
    }
    ...
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `### 🔊 Mass Voice Unmute Executed\n` +
          `-# *Server unmuted members in voice channel ${channel.name}.*\n\n` +
          `> - **Voice Channel:** <#${channel.id}>\n` +
          `> - **Members Unmuted:** \`${successCount}\` member(s)\n` +
